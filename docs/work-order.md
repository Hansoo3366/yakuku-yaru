...
- [x] MySQL 컨테이너 설정
- [ ] API 서버에서 MySQL 연결 확인
- [x] DB migration 또는 schema 초기화 방식 결정
- [x] `teams` seed 데이터 준비

추천 완료 기준:

- [ ] `docker compose up -d`로 MySQL이 실행된다. Docker Desktop 실행 후 확인 필요.
- [ ] API 서버에서 DB 연결 성공 로그가 나온다.
...
- [ ] 회원가입 가능. Docker Desktop 실행 후 DB 연결 검증 필요.
- [ ] 로그인 시 JWT 발급. Docker Desktop 실행 후 DB 연결 검증 필요.
- [ ] JWT로 보호된 API 호출 가능. Docker Desktop 실행 후 DB 연결 검증 필요.
- [ ] Swagger에서 Auth API 확인 가능. API 서버 실행 후 확인 필요.
...
- 사용자가 내 팀을 설정할 수 있다.
- 내 팀 경기 일정만 조회할 수 있다.
- 경기 상세에서 구장, 시간, 예매처, 예매 오픈 시간을 볼 수 있다.
...
- `/calendar`에서 월간 경기 일정이 보인다.
- 내 팀을 바꾸면 캘린더 일정이 바뀐다.
- 모바일 화면에서도 주요 흐름이 사용 가능하다.
...
- 경기별 직관 기록을 만들 수 있다.
- 사진을 업로드할 수 있다.
- 기록이 있는 날짜에 캘린더 썸네일이 보인다.
- 사용자가 기록을 수정/삭제할 수 있다.
...
- 마이페이지에서 직관 횟수, 승/패/무, 승률이 보인다.
- 승률이 50% 이상이면 `승리요정` 타이틀이 보인다.
...
- Docker production 설정
- Cloud Server 배포
- Swagger URL 확인
- GitHub repository 정리
- GitHub Actions 자동 배포
- README에 실행/배포 URL 정리

추천 완료 기준:

- 제출 가능한 GitHub Repository가 있다.
- 배포 URL이 있다.
- Swagger API 문서 URL이 있다.
