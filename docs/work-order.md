...
- [ ] 관리자 권한 모델 설계 (`users.role` 또는 별도 admin 정책)
- [ ] 관리자 로그인/접근 제어
- [ ] 사용자 목록/검색/상태 관리
- [ ] 게시글/댓글 목록 조회 및 삭제 관리
- [ ] 경기 일정/스코어/구장 정보 관리
...
- [ ] 일반 사용자는 `/admin`에 접근할 수 없다.
- [ ] 관리자는 사용자와 게시글 데이터를 검색하고 필요한 조치를 할 수 있다.
- [ ] 경기 일정 seed에 의존하지 않고 관리자 화면에서 주요 데이터를 관리할 수 있다.
